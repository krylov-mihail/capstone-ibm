...
expression_list = spacex_df[[]] 

launchSites = spacex_df['Launch Site'].unique()
for launchSite in launchSites:
    options.append({'label': launchSite, 'value':launchSite})
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Successful cases across all launch sites')
        return fig
    else:
        ...
        df_result.columns = ['Score', 'Total']

        fig = px.pie(df_result, values='Total', 
        names='Score' , 
        title='Success / Failure rate for '+ entered_site)
        return fig
